...
const pages = [
  ...
  },
  {
    ...
    sections: [
      ...
      ['Quais são os 5 sensos japoneses?', () => `<p><strong>Os 5 sensos japoneses são Seiri, Seiton, Seiso, Seiketsu e Shitsuke</strong>, traduzidos de forma prática como utilização, organização, limpeza, padronização e disciplina. Eles funcionam como um ciclo: decidir o que é necessário, definir onde fica, inspecionar enquanto limpa, tornar a condição esperada visível e mantê-la com constância.</p><h3>Seiri — utilização</h3><p>Separar o necessário do desnecessário. A pergunta central é: este item, material, documento ou etapa é necessário para o trabalho? O destino do excesso deve ser definido com responsabilidade, sem descarte improvisado.</p><h3>Seiton — organização</h3><p>Definir lugar, identificação e critério de reposição para o que ficou. Um bom arranjo reduz deslocamentos, buscas e dúvidas, respeitando ergonomia, segurança e frequência de uso.</p><h3>Seiso — limpeza</h3><p>Limpar e inspecionar ao mesmo tempo. Sujeira recorrente, vazamentos, peças soltas e resíduos fora do padrão podem revelar causas que precisam de correção.</p><h3>Seiketsu — padronização e saúde</h3><p>Consolidar condições adequadas por meio de padrões visuais, rotinas, responsáveis e frequências. O padrão deve ser claro e possível de cumprir.</p><h3>Shitsuke — disciplina</h3><p>O senso de disciplina no 5S significa manter o combinado, acompanhar desvios e melhorar o padrão quando necessário. Não é punição: depende de consistência, exemplo da liderança, recursos e aprendizagem.</p>`],
  ...
  },
  {
    ...
    sections: [
      ...
      ,['Uma sequência simples para testar', r => `<p>Comece registrando compromissos e tarefas por alguns dias. Em seguida, use a <a href="${r}produtividade/matriz-eisenhower/">Matriz de Eisenhower</a> para decidir o que fazer, planejar, delegar ou reduzir. Reserve na agenda um bloco realista para uma prioridade e experimente ciclos de foco se o tipo de trabalho permitir.</p><p>Para organizar suas tarefas nos quatro quadrantes, use a <a href="${r}ferramentas/matriz-eisenhower/">Matriz de Eisenhower online gratuita</a>.</p><p>Na revisão, avalie entrega, qualidade, energia e interrupções — não apenas quantidade de tarefas. Se o plano falhou, ajuste capacidade, ambiente ou tamanho da tarefa. Produtividade sustentável melhora resultados sem esconder sobrecarga nem transferir risco.</p>`]
  ...
  },
  {
    ...
    sections: [
      ...
      ['Gestão de resíduos sólidos em diferentes operações', r => `<p>A gestão de resíduos sólidos precisa refletir a atividade geradora. Em obras, as fases do canteiro alteram materiais, volumes e pontos de coleta. Na mineração, manutenção, operação, laboratórios e áreas administrativas geram fluxos distintos. O inventário deve registrar origem, tipo, condição e destino previsto, conectado aos controles de <a href="${r}mineracao/gestao-ambiental-na-mineracao/">gestão ambiental na mineração</a> quando esse for o contexto.</p>`],
      ['Identifique resíduos comuns e perigosos', () => `<p>A classificação orienta acondicionamento, compatibilidade, transporte e destinação. Resíduos perigosos demandam identificação e controles coerentes com suas propriedades; aparência ou cor do recipiente, sozinhas, não determinam a classificação. Use critérios legais e técnicos vigentes, informações do processo e documentos do produto. Em caso de incerteza, isole e busque avaliação competente.</p>`],
      ...
      ['Acondicionamento e armazenamento temporário', () => `<p>O armazenamento temporário não é um ponto de abandono: precisa de capacidade, prazo de retirada e responsáveis definidos.</p><ul class="practice-list"><li>Use recipientes íntegros e compatíveis com o conteúdo.</li><li>Proteja contra chuva, vento, vazamento e acesso indevido.</li><li>Separe materiais incompatíveis e respeite limites.</li><li>Mantenha identificação legível e área organizada.</li><li>Planeje inspeção, limpeza e resposta a incidentes.</li></ul>`],
...
function schemas(page) {
  ...
  const pagePublished = page.published || pageUpdated;
  ...
  const primary = page.kind === 'article' ? {
    ...
    datePublished:pagePublished, dateModified:pageUpdated, mainEntityOfPage:{'@type':'WebPage','@id':base+page.slug}, isPartOf:{'@id':base+'#website'}, image:base+'assets/images/adonias-profile-400.jpg', inLanguage:'pt-BR'
  } : page.kind === 'about' ? {
...
function render(page) {
  ...
  const authorBio = page.verifiedAuthor ? 'Técnico em Meio Ambiente com experiência prática em projetos de construção, mineração, 5S, gestão ambiental, produtividade e tecnologia aplicada.' : 'Técnico em Meio Ambiente, com atuação prática em grandes projetos de construção e mineração. Conteúdo sobre 5S, produtividade, meio ambiente e tecnologia aplicada.';
  const authorBox = `<aside class="author-box" aria-label="Sobre o autor"><img src="${r}assets/images/adonias-profile-200.jpg" srcset="${r}assets/images/adonias-profile-400.jpg 2x" width="72" height="72" loading="lazy" decoding="async" alt="Adonias Pereira da Silva"><div><h2>Conteúdo por ${author}</h2><p>${authorBio} <a href="${r}sobre/">Conheça o autor e a política editorial</a>.</p></div></aside>`;
...
const sitemap = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${sitemapUrls.map(slug => `  <url><loc>${base}${slug}</loc><lastmod>${slug === 'ferramentas/matriz-eisenhower/' ? '2026-09-13' : slug === 'ferramentas/gestao-do-tempo/' ? '2026-09-02' : (pageBySlug.get(slug)?.updated || updated)}</lastmod></url>`).join('\n')}\n</urlset>\n`;
