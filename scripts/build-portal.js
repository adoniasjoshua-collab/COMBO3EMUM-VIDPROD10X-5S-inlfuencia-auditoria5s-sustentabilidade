...
const pages = [
  ...
  },
  {
    ...
    sections: [
      ...
      ['Organize também o ambiente', r => `<p>Tempo se perde quando materiais, informações e versões não são encontrados. Use os princípios de <a href="${r}produtividade/5s-e-produtividade/">5S e produtividade</a> para reduzir atrito físico e digital.</p>`]
...
function schemas(page) {
  ...
  const primary = page.kind === 'article' ? {
    '@type':'Article', headline:page.h1, description:page.description, author:{'@type':'Person', name:author, url:base+'sobre/'},
    datePublished:updated, dateModified:updated, mainEntityOfPage:{'@type':'WebPage','@id':base+page.slug}, image:base+'assets/images/adonias-profile-400.jpg', inLanguage:'pt-BR'
  } : page.kind === 'about' ? {
    '@type':'ProfilePage', mainEntity:{'@type':'Person', name:author, jobTitle:'Técnico em Meio Ambiente', url:base+'sobre/', image:base+'assets/images/adonias-profile-400.jpg'}, dateModified:updated, inLanguage:'pt-BR'
  } : {
    '@type':'CollectionPage', name:page.h1, description:page.description, url:base+page.slug, dateModified:updated, inLanguage:'pt-BR'
  };
  const graph = [primary, {'@type':'BreadcrumbList', itemListElement:crumbs}];
  if (page.slug === 'portal/') graph.push({'@type':'WebSite', name:'Portal 5S e Produtividade', url:base+'portal/', inLanguage:'pt-BR', publisher:{'@type':'Person', name:author}});
...
const sitemap = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${sitemapUrls.map(slug => `  <url><loc>${base}${slug}</loc><lastmod>${updated}</lastmod></url>`).join('\n')}\n</urlset>\n`;
