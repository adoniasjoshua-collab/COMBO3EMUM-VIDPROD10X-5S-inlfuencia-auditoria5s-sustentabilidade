...
const activityTimeRows = reports.activityTimeBreakdown(report);
assert.equal(activityTimeRows.length, 1, 'grafico retorna atividades com tempo registrado');
assert.equal(activityTimeRows[0].trackedMs, 45_000, 'grafico preserva o tempo registrado da atividade');
assert.equal(Object.hasOwn(activityTimeRows[0], 'plannedMs'), false, 'grafico por atividade nao compara tempo planejado');
...
assert.equal((toolPage.match(/tempo10x\/[a-z]+\.js\?v=20260831-5/g) || []).length, 7, 'modulos usam cache-buster consistente');
assert.ok(toolPage.includes('tempo10x.css?v=20260831-5'), 'estilos usam cache-buster consistente');
