...
(async () => {
  ...
  try {
    ...
    const add = async (title, quadrant = 'plan') => { await page.locator('#eisenhower-add').click(); await page.locator('[name="title"]').fill(title); await page.locator('[name="quadrant"]').selectOption(quadrant); await page.locator('textarea[name="description"]').fill('Revisar material e combinar a próxima ação.'); await page.locator('[name="category"]').fill('Trabalho'); await page.locator('[name="dueDate"]').fill('30/09/2026'); await page.getByRole('button', { name: 'Salvar tarefa', exact: true }).click(); };
    ...
    await page.locator('[data-action="edit"]').click(); await page.locator('[name="title"]').fill('Preparar treinamento da equipe'); await page.locator('[name="status"]').selectOption('in_progress'); await page.getByRole('button', { name: 'Salvar tarefa', exact: true }).click(); assert.equal(await read('[data-stat="in_progress"]'), '1');
    await page.locator('[data-action="toggle"]').click(); assert.equal(await read('#eisenhower-xp'), '7 XP');
    await page.locator('[data-action="toggle"]').click(); await page.locator('[data-action="toggle"]').click(); assert.equal(await read('#eisenhower-xp'), '7 XP');
    await page.reload(); await page.locator('.eisenhower-card').waitFor(); assert.equal(await read('#eisenhower-xp'), '7 XP');
    await page.locator('[data-move]').selectOption('do'); assert.equal(await page.locator('#eisenhower-list-do .eisenhower-card').count(), 1);
    await page.locator('[data-drag]').dragTo(page.locator('[data-quadrant="delegate"] h3')); assert.equal(await page.locator('#eisenhower-list-delegate .eisenhower-card').count(), 1);
    ...
    await page.locator('#eisenhower-goal').fill('5'); await page.locator('#eisenhower-goal').press('Tab'); assert.equal(await read('#eisenhower-today'), '1 / 5');
    const downloadPromise = page.waitForEvent('download'); await page.locator('#eisenhower-export').click(); const download = await downloadPromise; const backupPath = await download.path(); const backup = JSON.parse(fs.readFileSync(backupPath, 'utf8')); assert.equal(backup.tasks.length, 2);
    await page.locator('[data-action="delete"]').first().click(); await page.locator('#eisenhower-confirm-cancel').click(); assert.equal(await page.locator('.eisenhower-card').count(), 2);
    await page.locator('[data-action="delete"]').first().click(); await page.locator('#eisenhower-confirm-ok').click(); assert.equal(await page.locator('.eisenhower-card').count(), 1);
    ...
    await page.reload(); await page.locator('.eisenhower-card').first().waitFor(); assert.equal(await page.locator('#eisenhower-goal').inputValue(), '5');
    await add('<img src=x onerror=alert(1)>', 'eliminate'); assert.equal(await page.locator('.eisenhower-card img').count(), 0);
    await page.locator('#eisenhower-list-eliminate [data-action="edit"]').click(); await page.locator('[name="title"]').fill('Avaliar relatório duplicado'); await page.getByRole('button', { name: 'Salvar tarefa', exact: true }).click();
    ...
    await page.setViewportSize({ width: 390, height: 844 }); await page.locator('#eisenhower-list-do [data-move]').selectOption('plan');
    ...
    await page.locator('#eisenhower-clear').click(); await page.locator('#eisenhower-confirm-cancel').click(); assert.equal(await page.locator('.eisenhower-card').count(), 4);
    await page.evaluate(() => localStorage.setItem('tempo10x.test-sentinel', 'keep'));
    await page.locator('#eisenhower-clear').click(); await page.locator('#eisenhower-confirm-ok').click(); assert.equal(await page.locator('.eisenhower-card').count(), 0); assert.equal(await read('#eisenhower-xp'), '0 XP'); assert.equal(await page.evaluate(() => localStorage.getItem('tempo10x.test-sentinel')), 'keep');
    ...
    await page.locator('#eisenhower-clear').click(); await page.locator('#eisenhower-confirm-ok').click();
